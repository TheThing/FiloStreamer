                ...
                await this.Dispatcher.InvokeAsync(() =>
                {
                    switch (snapshotContext.Format)
                    {
                        case SnapshotFormat.BMP:
                            ...
                            using (Stream stream = File.Create(string.Format("{0}\\{1}.bmp", snapshotContext.Path, snapshotContext.Name)))
                            {
                                bmpE.Save(stream);
                            }
                            break;
                        case SnapshotFormat.JPG:
                            ...
                            using (Stream stream = File.Create(string.Format("{0}\\{1}.jpg", snapshotContext.Path, snapshotContext.Name)))
                            {
                        ...
                        case SnapshotFormat.PNG:
                            ...
                            using (Stream stream = File.Create(string.Format("{0}\\{1}.png", snapshotContext.Path, snapshotContext.Name)))
                            {
                                pngE.Save(stream);
                            }
                            break;
